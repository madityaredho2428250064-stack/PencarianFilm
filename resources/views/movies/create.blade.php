    ...
    <div class="py-8" style="background: linear-gradient(135deg, #0f0f1a 0%, #1a1130 100%); min-height: 100vh;">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8 shadow-xl">
                ...
                <form method="POST" action="{{ route('movies.store') }}" class="space-y-5">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea name="overview" rows="4"
                                  class="w-full rounded-lg bg-gray-900/50 border border-gray-700 text-gray-100 px-4 py-2 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition">{{ old('overview') }}</textarea>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-300 mb-1">URL Poster</label>
                        <input type="text" name="poster_path" value="{{ old('poster_path') }}"
                               placeholder="https://..."
                               class="w-full rounded-lg bg-gray-900/50 border border-gray-700 text-gray-100 px-4 py-2 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition">
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.1" min="0" max="10" name="rating_avg" value="{{ old('rating_avg', 0) }}"
                                   class="w-full rounded-lg bg-gray-900/50 border border-gray-700 text-gray-100 px-4 py-2 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        @if ($genres->isEmpty())
                            <p class="text-sm text-gray-500">Belum ada genre. Tambahkan genre dulu lewat database/seeder.</p>
                        @endif
                    </div>
